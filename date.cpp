...
{
    int total_day_d1 = d1.get_month_day() + d1.get_year()*365;
    if(Date::isleap(d1.get_year())) {
        pmonthDays[2] = 29;
    } else {
        pmonthDays[2] = 28;
    }
    for(int i = 1; i < d1.get_month(); ++i) {
        total_day_d1 += pmonthDays[i];
    }

    int total_day_d2 = d2.get_month_day() + d2.get_year()*365;
    if(Date::isleap(d2.get_year())) {
        pmonthDays[2] = 29;
    } else {
        pmonthDays[2] = 28;
    }
    for(int i = 1; i < d2.get_month(); ++i) {
        total_day_d2 += pmonthDays[i];
    }
